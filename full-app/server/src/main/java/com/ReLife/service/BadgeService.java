...
public class BadgeService {
	  ...
	  public BadgeService(BadgeEventRepository repo) { this.repo = repo; }
	...
	public BadgeEvent awardBadge(Long userId, String badgeKey) {
		  ...
		  // build new hash: SHA-256(prevHash ∥ badgeKey ∥ timestamp)
		  String timestamp = Instant.now().toString();
		  String concatenated = prevHash + "|" + badgeKey + "|" + timestamp;
		  String newHash = DigestUtils.sha256Hex(concatenated);

		  BadgeEvent ev = new BadgeEvent();
		  ev.setUserId(userId);
		  ev.setBadgeKey(badgeKey);
		  ev.setTimestamp(Instant.parse(timestamp));
