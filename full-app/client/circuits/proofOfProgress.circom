...
// ^ Powers of Tau ceremony of size 2¹² (ptau) is too big
...
template PoseidonMerkle(depth) {
    ...
    for (var i = 0; i < depth; i++) {
        // Select ordering based on pathIndices[i]
        // If pathIndices[i] == 0: (leaf, sibling)
        // If pathIndices[i] == 1: (sibling, leaf)
        signal left;
        signal right;

        left  <== pathIndices[i] * pathElements[i] + (1 - pathIndices[i]) * hash[i];
...
component main = PoseidonMerkle(3);