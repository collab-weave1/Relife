...
export const Login = ({ onLogin, isDark }) => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      let userId = result?.user?.id ?? result?.session?.user?.id ?? null;
      ...
      if (userId) {
        try {
          const { data: profileData, error: profileErr } = await supabase
            ...
            let userRole = result.user.user_metadata.role
            console.log("result",userRole,userRole.role);
        ...
        //     finalRole = profileData.role;
            ...
            if (userData?.user?.user_metadata?.role) {
              finalRole = userData.user.user_metadata.role;
            }
          }
        } catch (err) {
          console.warn("Could not fetch profile after signIn:", err);
        }
      } else {
        console.warn("User ID not available after signIn; using selected role");
      }
      
      console.log(result, await supabase.auth.getSession(), await supabase.auth.getUser())
        ...
        <div className="relative z-10 flex flex-col justify-center p-8 text-white">
          <div className="max-w-md">
            ...
            <p className="text-2xl font-bold text-white/90 mb-8 leading-relaxed">
              Join thousands making a difference in the e-waste revolution. Every device recycled is a step toward a sustainable future.
            </p>
        ...
        <div className="w-full max-w-md">
          ...
          <div className="text-center mt-8">
            <p className="text-gray-600 dark:text-gray-400 text-sm">
              ...
              <button onClick={() => navigate("/signup")} className="text-green-600 dark:text-green-400 hover:text-green-700 dark:hover:text-green-300 font-semibold transition-colors">
                Sign up
              </button>
            </p>
          </div>
